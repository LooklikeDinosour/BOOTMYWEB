...
//		this.end =(int)(Math.ceil(현재 조회하는 페이지/ 페이지네이션 갯수)) * 페이지네이션 갯수;
...
//		this.realEnd = (int)(Math.ceil(전체 게시글 수 / 데이터개수));
		this.realEnd = (int)(Math.ceil(this.total / (double)this.amount));
		
		//end페이지 재결정
		//데이터가 25개 -> end = 10, realEnd = 3 (실제 끝번호를 따라감) realEnd < end => realEnd
		//데이터가 153개 -> end = 20, realEnd = 16 (실제 끝번호를 따라감) realEnd < end => realEnd
		//데이터가 153개(3번페이지 조회시) -> end = 10, realEnd = 16(페이지네이션 따라감) realEnd > end => end
		...
		//next활성화 여부
		//end = 10, realEnd = 16 라고할 때 다음 버튼이 있다는 의미
		this.next = this.realEnd> this.end;
		
		//타임리프 - 리스트에 페이지네이션을 다음
